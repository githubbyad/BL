...
function GetTimeAgo(sDate, adjusttime, Article_Timestamp, format, TimezoneOffset) {
	...
	else if (hours > 0) str = document.write(hours + ' hours ago'); 
	else if (mins > 0) str = document.write(mins + ' mins ago');
	else if (secs >= 0) str = document.write(secs + ' secs ago');
...
function OpenLoginPopup(ctl) {
        ...
        function checkLoginStatus(isRestricted) {
            ...
            if (sname.length > 0) {
				if (urlhttps == true) {
                var sframe = "<iframe id=\"IFrameContent\" name=\"IFrameContent\" marginwidth=\"0\" marginheight=\"0\" hspace=\"0\" vspace=\"0\" frameborder=\"0\" scrolling=\"no\" width=100% onload=\"ResizeIFrame('IFrameContent');\" src=\"https://" + location.href.match(/:\/\/(.[^/]+)/)[1] + "/" + sname + qs + "\"></iframe>"
				}
				else
				{
                var sframe = "<iframe id=\"IFrameContent\" name=\"IFrameContent\" marginwidth=\"0\" marginheight=\"0\" hspace=\"0\" vspace=\"0\" frameborder=\"0\" scrolling=\"no\" width=100% onload=\"ResizeIFrame('IFrameContent');\" src=\"http://" + location.href.match(/:\/\/(.[^/]+)/)[1] + "/" + sname + qs + "\"></iframe>"
				}
