        ...
        if (readCookie('cart') != null) {
            ...
            for (var y = 1; y <= l; y++) {
                ...
                if (dc.startsWith('1-') == true) {
                    pv = parseFloat(pv).toFixed(2).replace(/[^\d\.\-]/g, "");
                    fdc = dc.split('|')[0].split('-')[2];
                    var fdc_d = parseFloat(fdc).toFixed(2) / 100;
                    var fdc_m = pv * fdc_d;
                    pv = pv - fdc_m;
                    pv = parseFloat(pv).toFixed(2).replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
                }
                $(".checkout-load ol").append("<li><div class='li-line1'><span class='hidden-pimg'>" + p + "</span><div class='productx-img' data-img='" + p + "'><img src='" + p + "'></div></div><div class='li-line2'><div class='productx-name'>" + i + "</div><span class='productx-curr'>" + c + "</span><span data-value='" + v + "' class='eachPrice productx-price'>" + pv + "</span><div class='productx-dc' data-dsc='" + fdc + "' data-discount='" + dc + "'><span><b>" + fdc + "</b>% Discount Applied</span></div><div class='qty-count' data-sf='" + sf + "' data-sfx='" + sf + "' data-dscx='" + dc + "' data-dscl='" + dc_last + "'><div class='qty-div'>Quantity: </div><select name='qty' id='qty' class='qty'></select></div><div class='sx-weight' style='display:none;'>" + w + "</div><div title='Click to Remove' class='productx-remove'><img src='/lib/images/trash2.png'></div></li>");
            }
            for (qi = 1; qi <= 500; qi++) {
                $('.qty').each(function() {
...
$(window).bind('load', function() {
    if (window.location.href.indexOf("/cartcheckout.htm") != -1) {
        ...
        var a = setInterval(function() {
            ...
            $('.li-line2').each(function() {
                // var vq = $(this).children('.productx-price').attr('data-value').replace(/[^\d\.\-]/g, "");
                $(this).children('.qty-count').children('#qty').change(function() {
                    ...
                    $(this).parents('.qty-count').siblings('.productx-dc').css('display', 'none');
                    if ($(this).val() <= parseInt($(this).parents('.qty-count').attr('data-dscl'))) {
                        ...
                        for (i = 0; i < dsc_length; i++) {
                            ...
                            if (dsc_qty >= parseInt(min) && dsc_qty <= parseInt(max)) {
                                ...
                                // $(this).parents('.qty-count').siblings('.productx-dc').attr('data-vq', vq);
                                $(this).parents('.qty-count').siblings('.productx-dc').attr('data-min', min);
                                $(this).parents('.qty-count').siblings('.productx-dc').attr('data-max', max);
                                $(this).parents('.qty-count').siblings('.productx-dc').attr('data-i', i);
                            }
                        }
                    }
